...
export const deleteProduct = async(req,res)=>{
  ...
  if((req.role!='admin')&&(req.Id!=foundProduct.createdBy))  return res.json({message:"You are not allowed to delete this product"});
...
//console.log(req.Id,req.role);
  ...
  let foundProduct = await productModel.findById(req.params.productId);
  if(!foundProduct) return res.json({message:"product not found"});
  if((req.role!='admin')&&(req.Id!=foundProduct.createdBy))  return res.json({message:"You are not allowed to update this product"});
  ...
  let updatedProduct = await productModel.findByIdAndUpdate(foundProduct._id,data,{new:true})
...
export const getAllProductsWithPagination= async(req,res)=>{
  ...
  if(req.query.page){
    ...
    if(skip>= productsCount){
     ...
     const  pageProducts= await productModel.find({}).skip(skip).limit(limit).exec();
...
export const getAllWithCategoryId = async(req,res)=>{
  ...
  if(!foundProducts) return res.json({message:"No products in this category"});
